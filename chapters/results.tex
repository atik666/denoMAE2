...
\begin{table*}[htbp]
    ...
    \toprule
    \multirow{2}{*}{\textbf{Methods}} & \multicolumn{2}{c}{\textbf{Number of samples}} & 
    \multirow{2}{*}{\textbf{SNR dB}} & \multirow{2}{*}{\textbf{Number of}} 
    & \multirow{2}{*}{\textbf{Finetuning}} & \multirow{2}{*}{\textbf{Inference time}} 
    & \multirow{2}{*}{\textbf{Test accuracy}} \\ 
    \cmidrule(lr){2-3}
     & \textbf{Pretraining} & \textbf{Fine-tuning} & & \textbf{classes} & \textbf{parameters} 
     & \textbf{(ms)} & \textbf{(\%)} \\ \midrule
    ...
    DenoMAE \cite{faysal2025denomae} & 10,000 & 1,000 & 0 - 10 & 10 & 85.8M & 14.5  & 81.3 \\
    DenoMAE2.0 (\textbf{Ours}) & 10,000 & 1,000 & 0 - 10 & 10 &  85.8M  & 14.5  & 82.4 \\
...
We use a subset of the RadioML 2018.01A dataset~\cite{o2018over}, which is widely used as a benchmark in wireless communication~\cite{ccamlibel2024automatic} and spectrum sensing research~\cite{elyousseph2021deep}, facilitates advancements in deep learning-based modulation recognition, interference mitigation, and cognitive radio applications~\cite{cheng2024automatic, jagatheesaperumal2024deep, 10014805}. The dataset consists of I/Q (in-phase and quadrature) signal samples, each with shape $1024 \times 2$. To evaluate performance across a comprehensive range of signal conditions, we selected five strategically chosen SNR levels: -20 dB, -10 dB, 0 dB, 10 dB, and 20 dB. This selection enables both interpolation analysis (at 0 dB and 10 dB, within DenoMAE2.0's pretraining range) and extrapolation assessment (at -20 dB, -10 dB, and 20 dB), providing insights into the model's generalization capabilities beyond its training distribution. For each modulation-SNR combination, we utilized 4,000 training samples, 46 validation samples, and 50 testing samples, balancing statistical significance with computational efficiency. In our study, we focus on the normal modulation types of RadioML dataset which consists of 11 classes as listed in Table~\ref{tab:radioml_classes_compact}.

\begin{table}[h]
    \centering
    \caption{Modulation classes in the RadioML}
    \small
    \begin{tabular}{p{5cm}l}
        \hline
        \textbf{Class Name} & \textbf{Abbreviation} \\ \hline
        4-level Amplitude Shift Keying & 4ASK \\ 
        8-phase Phase Shift Keying & 8PSK \\ 
        16-point Quadrature Amplitude Modulation & 16QAM \\ 
        Amplitude Modulation, Double Sideband Suppressed Carrier & AM-DSB-SC \\ 
        Amplitude Modulation, Single Sideband Suppressed Carrier & AM-SSB-SC \\ 
        Binary Phase Shift Keying & BPSK \\ 
        Frequency Modulation & FM \\ 
        Gaussian Frequency Shift Keying & GFSK \\ 
        Offset Quadrature Phase Shift Keying & OQPSK \\ 
        On-Off Keying & OOK \\ 
        Quadrature Phase Shift Keying & QPSK \\ \hline
    \end{tabular}
    \label{tab:radioml_classes_compact}
\end{table}

\subsection{Performance}

To evaluate transferability, we applied our models pretrained on the original dataset to the RadioML benchmark through fine-tuning. This process tests whether the representations learned during pretraining generalize effectively to new signal distributions and classification tasks. Figure~\ref{fig:trans} illustrates classification accuracy across multiple SNR levels, demonstrating how transfer learning performance scales with signal quality.

At extremely low SNRs (-20 dB), all three methods converge to the same baseline performance (9.09\%), indicating they reach a fundamental classification limit where signal features become indistinguishable from noise. This represents random guessing for an 11-class problem, suggesting that no model can extract meaningful patterns at this noise level. At moderately low SNR (-10 dB), DenoMAE2.0 begins to demonstrate its superior noise robustness by achieving 10.12\% accuracy while both DenoMAE and ViT remain at baseline performance. This small but notable improvement (1.03 percentage points) represents the earliest evidence of DenoMAE2.0's enhanced feature extraction capabilities in highly degraded signals.

The most performance differentiation occurs at 0 dB SNR, where DenoMAE2.0 achieves 39.53\% accuracy, outperforming DenoMAE by 3.2 percentage points and ViT by 6.13 percentage points. This substantial gap at the noise-signal equilibrium point highlights DenoMAE2.0's effectiveness precisely where denoising capabilities matter most. In favorable SNR conditions (10-20 dB), all models achieve high accuracy (\textgreater 90\%), with DenoMAE2.0 maintaining its lead but with narrowing margins. The relative improvements become more modest (approximately 1 percentage point at 10 dB and 0.79 percentage points at 20 dB compared to DenoMAE), suggesting that when signals are cleaner, sophisticated denoising techniques offer diminishing returns.

The consistent hierarchy of performance (DenoMAE2.0 \textgreater DenoMAE \textgreater ViT) across the entire operational SNR range confirms that the architectural enhancements in DenoMAE2.0 provide systematic benefits for signal classification. Most importantly, the performance advantage is proportionally largest when signal quality is poor, precisely the condition where robust performance is most valuable in practical applications.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{images/radioML.png}
