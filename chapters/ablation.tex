...
\begin{table}[htbp]
    \centering
    \caption{Accuracy results for different weight values}
...
We examined the impact of varying MLP hidden dimensions on model performance, with results presented in Table~\ref{tab:mlp_accuracy}. The experiments show that increasing the hidden dimensions from 64 to 256 leads to steady improvement in accuracy, reaching the peak performance of 82.40\% at 256 dimensions. However, further increasing the dimensions beyond 256 results in slightly diminished performance due to overfitting in our low data regime, with accuracy gradually decreasing to 82.00\% at 1024 dimensions. This suggests that while sufficient network capacity is important, extremely large hidden dimensions do not provide additional benefits and may lead to unnecessary computational overhead.

\begin{table}[htbp]
    \centering
    \caption{Accuracy results for different MLP sizes}
    \resizebox{\columnwidth}{!}{%
    \begin{tabular}{ccccccc}
        \toprule
        MLP dimensions & 64 & 128 & 256 & 512 & 768 & 1024 \\
        \midrule
        Accuracy (\%) & 81.00 & 81.60 & 82.40 & 82.20 & 82.00 & 82.00 \\
        \bottomrule
    \end{tabular}%
    }
